...
- [x] **9.5.1 damage-композитор (v0.9.8)**: off-screen back buffer (`wm_compose`) +
  частичные обновления только изменившихся прямоугольников (`wm_window_bounds`);
  движение курсора больше не перерисовывает весь экран. Вывод пиксельно идентичен
  полной перерисовке (0 различий).
- [x] **9.6 Dock как отдельный процесс — ПОДТВЕРЖДЕНО в QEMU (v0.9.9)**: Dock
  (`user/dock.c`) больше не рисуется из `desktop.c`, а является первым standalone
  GUI-клиентом. Borderless-окно (`WM_F_DOCK`): сервер закрепляет его снизу-по-центру,
  держит всегда сверху и исключает из клавиатурного фокуса; скруглённая панель
