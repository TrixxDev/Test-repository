...
Это **версия 0.5** — интерактивная Unix-подобная среда: ядро запускает
**shell** как первый пользовательский процесс. Shell читает команды со
stdin, разбирает их и запускает программы с диска через `fork`+`exec`+`wait`
(с передачей `argv`), поддерживает builtins (`help`, `exit`) и фоновые
процессы (`&`). Системный контракт (номера syscall) зафиксирован в едином
заголовке `include/syscall_abi.h` для ядра и userland.
...
user/           пользовательские программы (собираются отдельно)
  ...
  sh.c          интерактивный shell (первый процесс)
  hello.c       демо-программа (печатает argv)
