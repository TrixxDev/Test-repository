...
Это **версия 0.9.0** — ОС с сервисной моделью, сокетами loopback, базовой
безопасностью, записью на диск (FAT32 read/write) и **первым графическим
стеком**: линейный фреймбуфер, 2D-библиотека и статичный рабочий стол (обои +
верхняя панель + Dock). Конечная цель — desktop-ОС в духе macOS. Ядро запускает
...
- **2D-библиотека** (`kernel/gfx.c`) — прямоугольники, скругления, круги,
  вертикальный градиент, blit (пока без альфы и шрифтов).
- **Рабочий стол** (`kernel/desktop.c`) — обои-градиент, верхняя панель с
  логотипом, Dock с иконками. Рисуется один раз из `kmain`.
