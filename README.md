...
Это **версия 0.8.2** — ОС с сервисной моделью, сокетами loopback, базовой
безопасностью и **записью на диск (FAT32 read/write)**. Конечная цель —
desktop-ОС в духе macOS, поэтому следующий рывок — **визуальный стек**
(фреймбуфер → композитор → рабочий стол). Ядро запускает **init (PID 1)**,
который поднимает **logger**, **netd** и **shell**. Есть **message-passing IPC**,
**реестр сервисов** с правами, **сокеты `AF_LOOPBACK`** через `netd`, **uid +
rwx на VFS**, привилегированные порты, и **запись файлов на диск**
(`save`/`cat`). Архитектура зафиксирована в [`docs/`](docs/).
