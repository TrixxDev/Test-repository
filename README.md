...
Это **версия 0.2** — рабочее ядро с управлением памятью и многозадачностью:
оно загружается, поднимает память и paging, кучу, вытесняющий планировщик
и умеет переходить в пользовательский режим (ring 3) с системными вызовами.
...
arch/i386/      архитектурно-зависимый код (x86)
  ...
  paging.*      виртуальная память / paging (рекурсивное отображение)
  switch.S      переключение контекста задач
  usermode.S    вход в ring 3 + демо-программа пользователя
