...
| 19.1 | **Kernel stack guard pages** (Phase 19, Platform Hardening -- opened after 18.5 closed the network arc): every thread's kernel stack gets its own page-mapped slot with an unmapped guard page below it, instead of an ordinary `kmalloc()` block next to other heap objects. Motivated directly by a real bug already hit once (`fs/fat32.c`'s double-`cbuf` stack overflow, fixed at the time with zero protection added). Found and fixed a genuine architectural gotcha along the way: a same-privilege page fault whose own trigger is "ESP itself is unmapped" always cascades page-fault → double-fault → triple-fault with no diagnostic at all (confirmed via QEMU's own exception trace) -- fixed with a task-gate double-fault handler on a dedicated stack, the standard i386 technique for exactly this. `tools/guard_page_qemu.py` is a permanent acceptance test | — | ✅ full regression pass (all host tests + `prof_qemu`/`dns_cache_qemu`/`tls_resume_qemu` + the original bug's exact `save`-then-exit scenario) |
| 19.2 | **Stack canaries, three layers** (the in-page complement to 19.1's boundary): `-fstack-protector-strong` kernel- AND userspace-wide (was pinned off in both `CFLAGS` and `UCFLAGS`) with freestanding runtimes (`arch/i386/stack_protector.c`, `user/libc/ssp.c` — the userspace one covers the whole TLS/HTTP2 stack, exactly the code parsing hostile input); a PER-THREAD canary (hash of boot-TSC seed + tid + stack base, no rand()) swapped into `__stack_chk_guard` by `do_switch()` at every context switch (Linux-!SMP-style, so one thread's canary says nothing about another's); a stack-END canary word directly above the guard page checked at context switch / syscall exit / `thread_free()`; plus a `kstack_max_used` high-water-mark counter in `kernel_prof`. New `SYS_DEBUG_STACK_SMASH` + `user/canarytest.c` + permanent `tools/canary_qemu.py` (userspace smash → child exits 134 and the OS survives; kernel smash → "KERNEL STACK SMASHING DETECTED" halt, asserted to be the canary and not 19.1's guard page) | — | ✅ canary_qemu + guard_page_qemu both ALL PASS; all 24 host suites; QEMU under real scheduler load (`prof`/`tcp_sendwin`/`keepalive`/`securehttps`/`h2_reuse` + `h2_large`'s 1 MB scenario, a 40-min sustained run — its 5/20 MB scenarios not re-run: that pre-18.0 test sleeps fixed ~7.3 h of budgets, modernization logged in NEXT_STEPS) with the per-thread guard swap active — zero false positives |
